...
public class BooksDaoManager extends BaseDaoManager {
    ...
    private static final String dbPath = new File(Workspace.DATABASES_DIR, "library.db").getAbsolutePath();
    ...
    @Getter
    private Dao<BookArchive, String> archivesDao;
    private Dao<BookSerie, String> seriesDao;
    @Getter
    private Dao<BookChapter, String> chaptersDao;
    ...
    public BooksDaoManager() throws SQLException {
        super(dbPath);
        try {
            ...
            migrateSchema();
            logger.info("Connected to books database!");
        } catch (Exception e) {
            logger.error("Failed to connect to books database!", e);
        }
    }
    ...
    public synchronized <T> boolean save(T item) {
        if (item instanceof BookSerie bookSerie) {
            return createOrUpdate(seriesDao, bookSerie, bookSerie.getDbId() != null) > 0;
        } else if (item instanceof BookArchive bookArchive) {
            return createOrUpdate(archivesDao, bookArchive, bookArchive.getDbId() != null) > 0;
        } else if (item instanceof BookChapter bookChapter) {
            return createOrUpdate(chaptersDao, bookChapter, bookChapter.getId() != null) > 0;
        } else if (item instanceof Category category) {
            return createOrUpdate(categoryDao, category, category.getId() != null) > 0;
        } else {
