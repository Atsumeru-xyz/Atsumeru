...
public class FilteredBooksRepository {
    ...
    public static List<IBaseBookItem> getFilteredList(AtsumeruUser atsumeruUser, ContentType contentType, String category, LibraryPresentation libraryPresentation, String search,
        ...
        return getFilteredList(atsumeruUser, contentType, category, libraryPresentation, search, sort, ascending, status, translationStatus, plotType, censorship, color, ageRating,
    ...
    public static List<IBaseBookItem> getFilteredList(AtsumeruUser atsumeruUser, ContentType contentType, String category, LibraryPresentation libraryPresentation, String search,
        ...
        List<IBaseBookItem> list = BooksRepository.getBooks(atsumeruUser, libraryPresentation, 1, Integer.MAX_VALUE, false, false);

        Map<String, Category> allowedCategoriesMap = atsumeruUser.getAllowedCategoriesMap();
        ...
        if (sort == Sort.LAST_READ) {
            BooksRepository.loadVolumesAndChaptersInfo(atsumeruUser, libraryPresentation, filteredList, withVolumesAndHistory, withChapters, false);
            filteredList.sort(getLastReadComparator());
            cuttedFilteredList = filteredList.subList(subListFromIndex, subListToIndex);
        } else {
            cuttedFilteredList = filteredList.subList(subListFromIndex, subListToIndex);
            BooksRepository.loadVolumesAndChaptersInfo(atsumeruUser, libraryPresentation, cuttedFilteredList, withVolumesAndHistory, withChapters, false);
        }
    ...
    private static Comparator<IBaseBookItem> getSortComparator(Sort sort) {
        ...
        return switch (sort) {
            case TITLE -> TITLE_COMPARATOR;
            case YEAR -> ((Comparator<IBaseBookItem>) (item1, item2) -> AlphanumComparator.compareStrings(item1.getYear(), item2.getYear())).thenComparing(TITLE_COMPARATOR);
            case COUNTRY -> ((Comparator<IBaseBookItem>) (item1, item2) -> AlphanumComparator.compareStrings(item1.getCountry(), item2.getCountry())).thenComparing(TITLE_COMPARATOR);
            case LANGUAGE -> ((Comparator<IBaseBookItem>) (item1, item2) -> AlphanumComparator.compareStrings(item1.getLanguage(), item2.getLanguage())).thenComparing(TITLE_COMPARATOR);
            case PUBLISHER -> ((Comparator<IBaseBookItem>) (item1, item2) -> AlphanumComparator.compareStrings(item1.getPublisher(), item2.getPublisher())).thenComparing(TITLE_COMPARATOR);
            case SERIE -> getSerieComparator();
            case PARODY -> getParodyComparator();
            case VOLUMES_COUNT -> Comparator.comparingLong(IBaseBookItem::getVolumesCount).thenComparing(TITLE_COMPARATOR);
            case CHAPTERS_COUNT -> Comparator.comparingLong(IBaseBookItem::getChaptersCount).thenComparing(TITLE_COMPARATOR);
            case SCORE -> ((Comparator<IBaseBookItem>) (item1, item2) -> AlphanumComparator.compareStrings(item1.getScore(), item2.getScore())).thenComparing(TITLE_COMPARATOR);
            case CREATED_AT -> Comparator.comparingLong(IBaseBookItem::getCreatedAt).thenComparing(TITLE_COMPARATOR);
            case UPDATED_AT -> Comparator.comparingLong(IBaseBookItem::getUpdatedAt).thenComparing(TITLE_COMPARATOR);
            case POPULARITY -> Comparator.comparingInt(IBaseBookItem::getRating).thenComparing(TITLE_COMPARATOR);
            default -> Comparator.comparingLong(IBaseBookItem::getDbId);
        };
    }
    ...
    public static List<Filters> getFiltersList(ContentType contentType, String category, LibraryPresentation libraryPresentation) {
        List<IBaseBookItem> items = AtsumeruApplication.getContext().getBean(BooksDaoManager.class).queryAll(libraryPresentation.getDbClassForPresentation(), libraryPresentation);
    ...
    private static void safeAddCollectionIntoFiltersList(List<Filters> filters, Collection<String> collection, String id, boolean hasAndMode, boolean singleMode) {
        if (ArrayUtils.isNotEmpty(collection)) {
            filters.add(Filters.create(id, Localization.toLocale("filters." + id), hasAndMode, singleMode, new ArrayList<>(collection)));
        }
    }
