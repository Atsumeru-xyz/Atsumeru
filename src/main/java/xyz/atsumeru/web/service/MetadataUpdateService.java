...
import xyz.atsumeru.web.metadata.BookInfo;
...
public class MetadataUpdateService {
    ...
    private void injectAllFromDatabase() {
        List<BookArchive> archives = Beans.getBooksDaoManager().queryAll(BookArchive.class, LibraryPresentation.ARCHIVES);
    ...
    private void createUniqueIds(boolean insertIntoArchive, boolean insertIntoDBOnly, boolean forceCreate) {
        List<BookSerie> series = Beans.getBooksDaoManager().queryAll(BookSerie.class, LibraryPresentation.ARCHIVES);
        ...
        for (BookSerie serie : series) {
            ...
            serie.setSerieId(HashHelper.getMHash2(Constants.Hashes.SERIE_HASH_TAG, UUID.randomUUID().toString()));
            ...
            for (BookArchive archive : archives) {
                if (!archive.getIsUniqueContentID() || forceCreate) {
                    ...
                    List<History> historyList = Beans.getBooksDaoManager().getHistoryDao().queryByHash(archive.getContentId(), BookArchive.class);

                    // Create new Archive Hash
                    archive.setContentId(HashHelper.getMHash2(Constants.Hashes.ARCHIVE_HASH_TAG, UUID.randomUUID().toString()));
                    ...
                    if (isSaved) {
                        ...
                        if (!Settings.isDisableChapters()) {
                            archive.getChapters().forEach(chapter -> {
                                ...
                                Beans.getBooksDaoManager().save(chapter);
                            });
                        }

                        // Save new hashes into file attributes
                        Path archivePath = Paths.get(archive.getFolder());
                        FilesHelper.writeHashFileAttribute(archivePath, Constants.Hashes.ATTRIBUTE_HASH, archive.getContentId());
                        FilesHelper.writeHashFileAttribute(archivePath, Constants.Hashes.ATTRIBUTE_SERIE_HASH, serie.getContentId());
                        ...
                        for (History history : historyList) {
                            ...
                            Beans.getBooksDaoManager().getHistoryDao().save(history);
                        }
                    }
                }
            }
    ...
    private void saveIntoArchives(BookSerie bookSerie, boolean insertIntoArchive, boolean insertIntoDBOnly) {
        List<BookArchive> archives = Beans.getBooksDaoManager().query("SERIE", String.valueOf(bookSerie.getDbId()), BookArchive.class);
        resetCounters(archives.size());
        for (BookArchive archive : archives) {
