package xyz.atsumeru.web.importer;
...
import xyz.atsumeru.web.helper.ChapterRecognition;
import xyz.atsumeru.web.helper.Constants;
import xyz.atsumeru.web.helper.JavaHelper;
import xyz.atsumeru.web.importer.listener.OnImportCallback;
...
public final class Importer {
    ...
    public static java.util.logging.Logger fileLogger;
    ...
    public void init() {
        fileLogger = FileLogger.createLogger("ImporterLog", new File(Workspace.LOGS_DIR, "import.log"));
    }
    ...
    public static void calculateTotal(List<ImportFolder> properties) {
        incrementTotal(
                properties.stream()
                        .map(ImportFolder::getArchivesInFolder)
                        .mapToInt(List::size)
                        .reduce(0, Integer::sum)
        );
    }

    public static void importFile(ImportFolder property, File file, Map<String, BookSerie> seriesMap, Map<String, BookArchive> archivesMap, OnImportCallback callback) {
    ...
    public static void deleteNotInFileSystemBooks(List<IBaseBookItem> notInFileSystemBooks, Class<? extends IBaseBookItem> clazz) {
        ...
        if (ArrayUtils.isNotEmpty(bookPaths)) {
            int removed = Beans.getBooksDaoManager().removeByColumnIn("FOLDER", bookPaths, clazz);
            ...
            if (clazz.isAssignableFrom(BookArchive.class)) {
                List<String> serieIds = notInFileSystemBooks.stream()
                        ...
                        if (ArrayUtils.isNotEmpty(serieIds)) {
                            Beans.getBooksDaoManager().removeByColumnIn("ID", serieIds, BookSerie.class);
                        }
            }
            ...
            int removedChapters = Beans.getBooksDaoManager().removeByColumnIn("SERIE_ID", contentIds, BookChapter.class)
                    + Beans.getBooksDaoManager().removeByColumnIn("ARCHIVE_ID", contentIds, BookChapter.class);
    ...
    private static Pair<String, String> saveArchive(ReadableContent readableContent, Map<String, BookSerie> seriesMap, String parentPath, boolean reImportIfExist) {
        ...
        if (readableContent != null && (ArrayUtils.isNotEmpty(readableContent.getPageEntryNames()) || readableContent.isBookFile())) {
            BookArchive bookArchive = Beans.getBooksDaoManager().queryItem(readableContent.getBookArchive().getContentId(), BookArchive.class);
    ...
    private static BookSerie saveArchiveAndCreateSerie(ReadableContent readableContent, BookArchive bookArchiveInDb,
        ...
        if (bookSerie.getDbId() != null) {
            ...
            Beans.getBooksDaoManager().save(bookArchiveInDb);

            // Считаем новое количество томов в Серии
            bookSerie.setVolumesCount(Beans.getBooksDaoManager().countArchivesForSerie(bookSerie));
            Beans.getBooksDaoManager().save(bookSerie);
        } else {
            Beans.getBooksDaoManager().save(bookSerie);
            bookArchiveInDb.setSerie(bookSerie);
            Beans.getBooksDaoManager().save(bookArchiveInDb);
    ...
    private static long fixHistoryEntries(List<History> historyList, int pagesCount) {
        return historyList.stream()
                ...
                .peek(history -> {
                    ...
                    Beans.getBooksDaoManager().save(history);
                })
                .count();
    }
    ...
    private static void saveChaptersForArchive(ReadableContent archive, BookSerie bookSerie, BookArchive bookArchive, boolean reImportIfExist) {
        if (!Settings.isDisableChapters()) {
            ...
            if (ArrayUtils.isNotEmpty(chapters)) {
                ...
                Beans.getBooksDaoManager().save(bookArchive);
            }
        }
    }
    ...
    public static Map<String, BookSerie> getSeriesInFolderFromDb(ImportFolder property) {
        List<BookSerie> seriesList = Beans.getBooksDaoManager().queryLike(ImportService.FOLDER_FIELD_NAME, property.getPath(), BookSerie.class);
        return seriesList.stream()
                .peek(serie -> serie.setSerieUpdatedInImport(false))
                .collect(Collectors.toMap(BookSerie::getContentId, Function.identity()));
    }

    public static Map<String, BookArchive> getArchivesInFolderFromDb(ImportFolder property) {
        List<BookArchive> archivesList = Beans.getBooksDaoManager().queryLike(ImportService.FOLDER_FIELD_NAME, property.getPath(), BookArchive.class);
    ...
    public static void calculateVolumesAndChaptersCount(List<String> changedSeriePaths) {
        ...
        List<BookSerie> series = changedSeriePaths.stream()
                .map(seriePath -> Beans.getBooksDaoManager().queryLike(ImportService.FOLDER_FIELD_NAME, seriePath, BookSerie.class))
        ...
        series.forEach(serie -> {
            serie.setVolumesCount(Beans.getBooksDaoManager().countArchivesForSerie(serie));
            serie.setChaptersCount(Beans.getBooksDaoManager().countChaptersForSerie(serie));

            List<BookArchive> archives = Beans.getBooksDaoManager().query("SERIE", String.valueOf(serie.getDbId()), BookArchive.class);
            archives.forEach(archive -> {
                archive.setVolumesCount(serie.getVolumesCount());
                Beans.getBooksDaoManager().save(archive);
            });
            ...
            Beans.getBooksDaoManager().save(serie);
            incrementProgress();
            cliProgressBar.step();
        });
